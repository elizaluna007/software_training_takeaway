...
var _index = __webpack_require__(/*! ../../js_sdk/mmmm-image-tools/index.js */ 18); //
//
//
//
//
...
//
//
var _default = { data: function data() {return { key: '', button_key: '', login_on: __webpack_require__(/*! ../../static/eye.png */ 19), login_no: __webpack_require__(/*! ../../static/l_or_r.jpg */ 20), info: '', change_key: false, change_no_key: true, update_info: { name: '', sex: '', address: '', birthday: '', phone: '', headphoto: '' } };}, methods: { goto: function goto(url) {uni.navigateTo({ url: '/pages/login/login_username' });}, logout: function logout() {getApp().globalData.login_key = false;this.key = false;this.button_key = true;}, change: function change() {this.change_key = true;this.change_no_key = false;}, save: function save() {// console.log("开始打印修改后的信息");
      // console.log(this.info);
      uni.request({ method: "POST", // url: 'http://127.0.0.1:4523/m1/1437509-0-default/customer/changeCustomerInfo',
        url: 'https://v3710z5658.oicp.vip/customer/changeCustomerInfo', data: { name: this.info.name, sex: this.info.sex, defaultaddress: this.info.address, birthday: this.info.birthday, phone: this.info.phone, headphoto: this.info.headphoto }, header: { token: getApp().globalData.token }, success: function success(res) {// this.info = res.data;
          // console.log("修改信息成功");
        } });}, upload_img: function upload_img() {// uni.chooseImage({
      ...
      // 	}
      // });
      var that = this;uni.chooseImage({ count: 1, sizeType: ['original', 'compressed'], sourceType: ['album', 'camera'], success: function success(res) {console.log("选取图片成功");var tempFilePaths = res.tempFilePaths;(0, _index.pathToBase64)(tempFilePaths[0]) //图像转base64工具
          .then(function (base64) {console.log("开始验证");console.log(base64);console.log(typeof base64);that.avatar = base64; //将文件转化为base64并显示
            console.log("开始上传");console.log(base64);console.log("开始打印base64");console.log(JSON.stringify(base64));uni.request({ // url: 'http://127.0.0.1:4523/m1/1437509-0-default/shop/getAllShopInfo', //仅为示例，并非真实接口地址。
              url: 'https://v3710z5658.oicp.vip/customer/changeHeadphoto', //真实端口
              method: "POST", //不设置，默认为get方式
              data: { headphoto: JSON.stringify(base64) }, header: { token: getApp().globalData.token },
